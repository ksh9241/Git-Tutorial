...
- git rebase [branch-name] : 머지가 발생했을 때 머지커밋을 하지 않고 마스터 브랜치의 작업내용을 가져올 수 있다. 마스터 브랜치엔 아무 영향이 없다.
    - 마스터브랜치의 끝을 새로운 베이스로 만드는 작업이다.
    - feat 브랜치의 커밋이 얼마나 있던 상관없이 마스터브랜치 끝에 작업한 feat 브랜치의 커밋이 재생성되어 붙는다.
- git diff [A Branch]..[B Branch] : 두 브랜치 변경사항 비교하기.
...
### 브랜치 보호규칙 구성하기
- repository settings에서 설정할 수 있다. 
- 브랜치의 이름이 아닌 패턴을 통해서 생성을 관리할 수 있다. (대형 프로젝트의 경우)
- Require Pull Request 옵션은 PR을 승인할 때에만 커밋을 머지할 수 있다.


### Fork
- 수많은 사람이 프로젝트에 참여하는 경우 필요하다. 권한 없이도 누구나 기여를 할 수 있다.
- 다른 사람의 Repository를 내 Repository로 생성한다.
- 깃허브의 기능이다. 깃에는 존재하지 않는다.

### Git Rebase
- 깃 머지 대신 사용한다
- 깃의 히스토리를 지우는 작업에 사용한다.
    - 깃 리베이스와 깃 머지의 차이점
        - 여러 개발자와 작업을 할 경우 메인 브랜치에 푸시가 되면 다른 개발자는 작업을 하기위해 메인 브랜치의 수정사항을 합치게 되는데 이때 merge를 쓰면 합칠 때마다 머지커밋이 생성된다. 즉 히스토리가 더러워진다.
        - 이러한 문제를 리베이스는 해결해준다. 리베이스를 하면 깃 이력을 재작성한다. 리베이스란 feature 브랜치의 베이스를 새로 설정한다.
- 반드시 알아야하는 명령어는 아니다.