...
### Github 공동작업자 추가하기
- 공동 작업자는 변경 사항을 저장소에 푸시하도록 허용하는 것이다.
- 한쪽에서 일방적으로 추가하거나, 초대될 수 없다.
- 공동소유자는 커밋 등 저장소에 리모트할 수 있지만 리포지토리 세팅은 불가능하다.

### README란
- 프로젝트가 무슨 일을 하는지 설명하기 위한 파일이다.

### Git Gists
- 깃허브 지스트는 코드 조각을 간단히 공유하는 방법이다.
- 완전한 깃허브를 만들고 푸쉬하여 클론하는 방법은 번거롭다.
반면 지스트는 단순 코드나 자료를 올려서 공유할 수 있다.
- 사용방법
    - Github 로그인
    - 우측 상단에 나의 깃허브 클릭
    - your gists 클릭
    - 우측 상단 +버튼 클릭하여 지스트 추가
    - all gists클릭 후 다른 사람 gists 공유 가능

### Git pages
- 깃허브 밖으로 웹 페이지를 호스팅할 수 있게 해준다.
- 단, 정적 웹 페이지 외에는 아무것도 할 수 없다.
- 깃허브 페이지에는 두 가지 유형이 있다.
    - 사용자 사이트
        - 깃허브 계정당 하나씩으로 제한된다.
        - [사용자이름.github.io] 형식의 url 유형
    - 프로젝트 사이트
        - 우리가 만든 모든 깃허브 리포지토리가 호스팅된 웹사이트를 가질 수 있다.
        - [username.github.io/repo-name] 형식의 url유형

#### 깃허브 페이지 만들기
1. 깃허브 페이지를 만들 저장소를 만들거나 선택한다.
2. Settings 에 들어가서 제일 아래 GitHub Pages영역으로 이동한다.
3. 브랜치 혹은 특정 html을 지정한다.
4. 설정을 마치면 url을 알려준다.

### 협업 워크플로우
- 중앙 집중형 워크플로우
    - 단일 브랜치로 모든 사람들이 작업한다.
    - 여러 브랜치를 푸시, 풀하고 머지한다.
    - 단점 
        1. 3명의 개발자가 존재할 때 C가 2개의 커밋을 푸시한 상태에서 B가 1개를 커밋하고 푸시할 때 이미 저장소에는 푸시가 진행되어있어서 B의 푸시가 거부되는 현상이 발생.
        2. 미완성의 코드를 코드리뷰 받을 때에도 마스터 브랜치에서 작업하기 때문에 형상관리가 힘들다.

- 분산형 워크플로우 (피처 브랜치)
    - 메인 브랜치에서 작업하지 않고 브랜치를 만들어서 작업하여 푸시한다.
    - 장점
